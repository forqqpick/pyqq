...
class TEA():
    ...
    def encrypt(v, key):
        ...
        for i in xrange(filln):
            ...
            fills = fills + chr(248) #chr(random.randint(0, 255))
...
class Coder():
    ...
    def str2hexstr(data):
        ...
        #return ''.join(hex(struct.unpack('B',c)[0]).replace('0x','') for c in data)
    @staticmethod
    def hexstr2str(data):
    ...
    def genBytesHexstr(n):
        ...
        #return Coder.numlist2hexstr([1 for i in range(n)])
    @staticmethod
...
class HexPacket:
    ...
    def dump(self, n):
        ...
        while dc < n:
            ...
            if _c >= 32:
                _c = 32
            _data = self.data[self.cur+dc:self.cur+dc+_c]
            print " ".join("{}{}".format( _data[i], _data[i+1]) for i in range(0, len(_data), 2)).ljust(50) ,
            #print '>' + re.sub('\\r|\\n',' ', Coder.hexstr2str(_data).decode('utf8').encode(sys.getfilesystemencoding()))
            print '>' + re.sub('[\x00-\x1f]',' ', Coder.hexstr2str(_data))
            sys.stdout.flush()
            dc += _c
